...
def get_modified_residues(item, entity='all', as_cards=False):

    from sabueso.cards import ModifiedResidueCard, modified_residue_dict
    ...
    for feature in item['uniprot']['entry']['feature']:
        ...
        if feature['@type']=='modified residue':
            aux_dict=deepcopy(modified_residue_dict)
    ...
    if as_cards:
        output = [ModifiedResidueCard(ii) for ii in output]
