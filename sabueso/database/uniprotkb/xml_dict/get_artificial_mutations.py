...
def get_artificial_mutations(item, entity='all', as_cards=False):

    from sabueso.cards import MutationCard, mutation_dict
    ...
    for feature in item['uniprot']['entry']['feature']:
        ...
        if feature['@type']=='mutagenesis site':
            aux_dict=deepcopy(mutation_dict)
    ...
    if as_cards:
        output = [MutationCard(ii) for ii in output]
